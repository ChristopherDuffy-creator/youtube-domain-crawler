...
def settings() -> Settings:
    return Settings(
        watchlist_monthly_views=20_000,
        qualified_monthly_views=50_000,
        priority_monthly_views=100_000,
    )


def test_main_tiers_use_20k_50k_100k_and_require_an_evaluation_start() -> None:
    config = settings()
    assert determine_tier(10_000, True, "available", config) == "pending"
    assert determine_tier(20_000, True, "available", config) == "watchlist"
