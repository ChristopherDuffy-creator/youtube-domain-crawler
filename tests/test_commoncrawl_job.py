...
def test_commoncrawl_prefilter_is_retained_but_not_active() -> None:
    scheduler = build_scheduler(Settings())
    job_ids = {job.id for job in scheduler.get_jobs()}

    assert "commoncrawl_prefilter" not in JOB_FUNCTIONS
    assert "initial_commoncrawl_prefilter" not in job_ids
    assert "commoncrawl_prefilter" not in job_ids
    assert callable(run_commoncrawl_prefilter_job)
