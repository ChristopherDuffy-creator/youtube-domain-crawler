...
The approved controller has twelve evenly spaced UTC slots per day. Every production slot can screen up to 25 fresh unchecked domains and rerank them before at most five domains receive detailed backlink, source-traffic, link-verification and availability proof. The conservative envelope is hard-limited to $0.18/run, with a database-backed reservation ledger hard-limited to $2.16/UTC day. A full twelve-run day can deep-proof up to 60 domains; empty queues, an active proof lease, and exhausted budgets skip without provider calls. The free/cached ranking pool covers the 10,000-name daily dropped-domain feed so later slots do not starve after the first batch.
